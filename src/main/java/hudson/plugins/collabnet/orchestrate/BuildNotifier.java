...
import hudson.plugins.collabnet.browser.TeamForge.DescriptorImpl;
...
/**
 *  Notifies Orchestrate when a build is complete. This is job-specific configuration.
 */
public class BuildNotifier extends Notifier {

    /** The root URL to the Orchestrate or MQ server. */
    ...
    private static String LOG_MESSAGE_PREFIX = "TeamForge Orchestrate Build Notifier - ";
    ...
    /**
     * Creates a new BuildNotifier. Arguments are automatically supplied when the job configuration is read from the
     * configuration file.
     *
     * @param serverUrl the root of the Orchestrate application or MQ
     * @param serverUsername the username for authentication against the Orchestrate or MQ server
     * @param serverPassword the password for authentication against the Orchestrate or MQ server
     * @param sourceKey the association key for the source to publish to
     * @param ctfUrl The project homepage URL
     * @param ctfUser User id with traceability API access
     * @param ctfPassword Password for the user
     */
    @DataBoundConstructor
    public BuildNotifier(ConnectionFactory connectionFactory,String serverUrl, String serverUsername, Secret serverPassword, String sourceKey)
    {
    ...
    public String getServerPassword() {
        ...
        return (StringUtils.isBlank(plainTextPassword)) ? null : plainTextPassword;
    }

    /**
     * Reads the source key that identifies this server. Used by Jelly to render the UI template.
     *
    ...
    public boolean perform(AbstractBuild<?, ?> build, Launcher launcher, BuildListener listener)
        ...
        if (isBlank(getServerUrl())) {
            markUnstable(build, consoleLogger, "Build information NOT sent: the URL to the TeamForge Orchestrate server is missing.");
            return true;
        }

        if (isBlank(getSourceKey())) {
            markUnstable(build, consoleLogger, "Build information NOT sent: the source key for the TeamForge Orchestrate build source is missing.");
            return true;
        }

        try {
            initialize();
            log("Sending build information using " + orchestrateClient.getClass().getSimpleName(), consoleLogger);
            String json = converter.toOrchestrateAPI(build, getSourceKey().trim());
            orchestrateClient.postBuild(getServerUrl().trim(), getServerUsername(), getServerPassword(), json);
            log("Build information sent", consoleLogger);
        } catch (IllegalStateException ise) {
            markUnstable(build, consoleLogger, "Build information NOT sent: plugin needs a Jenkins URL (go to Manage Jenkins > Configure System; click Save)");
        } catch (Exception e) {
    ...
    /**
     * Jenkins (un)helpfully wipes out any initialization done in constructors or class definitions before
     * executing this #perform method. So we need to initialize it in case it wasn't already.
     */
    private void initialize() throws URISyntaxException {
        if (converter == null) {
            converter = new DefaultBuildToOrchestrateAPI(new DefaultBuildToJSON());
        }
     ...
     *
     * @param newConverter the new converter
     */
