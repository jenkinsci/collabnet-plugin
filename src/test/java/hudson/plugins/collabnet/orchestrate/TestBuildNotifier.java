...
public class TestBuildNotifier {
    ...
    public void setUp() {
        ...
        BuildNotifier.RadioConfig config = new BuildNotifier.RadioConfig(null, serverUrl,
                serverUsername, serverPassword, associationKey, "eventQ");
    ...
    public void performDoesNotTryToContactEmptyServer() throws Exception {
        // set up
        BuildNotifier.RadioConfig config = new BuildNotifier.RadioConfig(null,null,
                serverUsername, serverPassword, associationKey, "eventQ");
        notifier = new BuildNotifier(null,config);
        AbstractBuild build = mocks.createMock("build", AbstractBuild.class);
    ...
    public void performDoesNotTryToContactServerWithNoSourceKey() throws Exception {
        // set up
        BuildNotifier.RadioConfig config = new BuildNotifier.RadioConfig(null,"http://orchestrate.test",
                serverUsername, serverPassword, "","eventQ");
        notifier = new BuildNotifier(null, config);
        AbstractBuild build = mocks.createMock("build", AbstractBuild.class);
        printer.println("Starting to send build information to TeamForge EventQ...");
    ...
    public void performDoesNotTryToContactServerWithoutJenkinsRootURL() throws Exception {
        ...
        BuildNotifier.RadioConfig config = new BuildNotifier.RadioConfig(null,"http://orchestrate.test",
                serverUsername, serverPassword, sourceKey, "eventQ");
        notifier = new BuildNotifier(null, config);
        AbstractBuild build = mocks.createMock("build", AbstractBuild.class);
        BuildToOrchestrateAPI converter = mocks.createMock("converter", BuildToOrchestrateAPI.class);
        notifier.setConverter(converter);
    ...
    public void performUsesAmqpClientClass() throws Exception {
        // set up
        BuildNotifier.RadioConfig config = new BuildNotifier.RadioConfig(null,"amqp://mq.example.com:5672",
                serverUsername, serverPassword, "somekey", "eventQ");
        notifier = new BuildNotifier(null, config);
        AbstractBuild build = mocks.createMock("build", AbstractBuild.class);
        BuildToOrchestrateAPI converter = mocks.createMock("converter", BuildToOrchestrateAPI.class);
        notifier.setConverter(converter);
